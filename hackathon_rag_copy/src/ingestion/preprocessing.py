...
CHUNK_SIZE_WORDS = 100          # target size of a prose chunk
CHUNK_OVERLAP_WORDS = 30        # overlap between consecutive prose chunks
...
if __name__ == "__main__":
    INPUT_PDF = "hackathon_rag/src/assets/documents/malaria_book_pages_1_114.pdf"        # <- set this
    OUTPUT_JSON = "hackathon_rag/src/assets/documents/malaria_book_chunks_cleaned.json"     # <- and this

    if os.path.exists(INPUT_PDF):
        chunks = process_pdf(INPUT_PDF)
        save_chunks(chunks, OUTPUT_JSON)
    else:
        print(f"ℹ️  Set INPUT_PDF to a real file path (got: {INPUT_PDF})")
