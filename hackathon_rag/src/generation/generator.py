...
def clean_and_parse_llm_response(raw_output: str) -> dict:
    ...
    if "LLM response" in data:
        ...
        rec_field = response.get("Recommendation") or response.get("Recomendation", "")
        
        if isinstance(rec_field, str) and "```json" in rec_field:
            # Extract inner JSON from the Recommendation string
            inner_match = re.search(r'```json\s*(\{.*?\})\s*```', rec_field, re.DOTALL)
            if inner_match:
                try:
                    ...
                    if "LLM response" in inner_data:
                        ...
                        for key, value in inner_data["LLM response"].items():
                            ...
                            if value and (not response.get(key) or len(str(value)) > len(str(response.get(key, "")))):
                                response[key] = value
                except:
                    pass
            else:
        ...
        if "Recommendation" in response and isinstance(response["Recommendation"], str):
            # Remove any JSON-like artifacts
            response["Recommendation"] = re.sub(r'```json\s*|\s*```', '', response["Recommendation"])
            ...
            if response["Recommendation"].strip().startswith('{'):
                try:
                    ...
                    if "LLM response" in temp and "Recommendation" in temp["LLM response"]:
                        response["Recommendation"] = temp["LLM response"]["Recommendation"]
                except:
                    pass
...
def generate_answer(
    ...
    "{\n"
    '  "LLM response": {\n'
    '    "Recommendation": "Your direct factual response to the question. Must be plain text with [Page X] citations. DO NOT use markdown, code blocks, or nested JSON.",\n'
    '    "Evidence": "The exact verbatim excerpt from the provided evidence.",\n'
    '    "Citation": "Source metadata (e.g., [Page 13] or {\"page\": 13, \"section\": \"recommendations\"})",\n'
    '    "Confidence": "Choose exactly one: \'high\', \'low\', or \'insufficient\'"\n'
    "  }\n"
    "}\n\n"
    "### CRITICAL RULES ###\n"
    "1. The 'Recommendation' field MUST be plain text, NOT JSON, NOT markdown, NOT code blocks.\n"
    ...
    "{\n"
    '  "LLM response": {\n'
    '    "Recommendation": "Avoid artesunate + SP with co-trimoxazole [Page 13], and avoid artesunate + amodiaquine with efavirenz or zidovudine [Page 13].",\n'
    '    "Evidence": "In people who have HIV/AIDS and uncomplicated P. falciparum malaria, avoid artesunate + SP if they are being treated with co-trimoxazole...",\n'
    '    "Citation": "[Page 13]",\n'
    '    "Confidence": "high"\n'
    "  }\n"
    "}\n\n"
    ...
    try:
        ...
        try:
            ...
            raw_generated_ans = llm_response_dict.get("Recommendation") or llm_response_dict.get("Recomendation", raw_llm_output)
            
            # Clean up the answer text (remove any remaining code block markers)
            if isinstance(raw_generated_ans, str):
                raw_generated_ans = re.sub(r'```json\s*|\s*```', '', raw_generated_ans)
                ...
                if raw_generated_ans.strip().startswith('{'):
                    try:
                        ...
                        if "LLM response" in temp and "Recommendation" in temp["LLM response"]:
                            raw_generated_ans = temp["LLM response"]["Recommendation"]
                    except:
                        pass
            
            # Ensure we have all required fields
            if "Evidence" not in llm_response_dict or not llm_response_dict["Evidence"]:
                llm_response_dict["Evidence"] = chunk_evidence_text
            
            if "Citation" not in llm_response_dict or not llm_response_dict["Citation"]:
...
Good practice statement"""
    test_chunks = [{"text": test_context, "page": 13, "section": "recommendations"}]
    test_query = "Which five artemisinin-based combination therapies (ACTs) are recommended for treating uncomplicated P. falciparum?"
